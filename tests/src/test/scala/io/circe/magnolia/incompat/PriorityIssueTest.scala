...
/** Something weird is going on here. During derivation for the right side of
  * the map, Magnolia notices default list codec only after it partially derives
  * it as coproduct. Ending JSON is very bizarre:
  * {{{
  * {
  *   "theMap" : {
  *     "f" : {
  ...
  * }
  * }}}
  * This can be fixed by explicitly importing Encoder companion into scope (see
  * 2nd test).
  *
  * Seems like magnolia macro in some cases has higher priority than companion
  * provided implicits, that are not directly imported in scope
  */
